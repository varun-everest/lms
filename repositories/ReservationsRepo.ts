...
const createReservation = async(newReservation: any) => {
    try {
...
const updateReservation = async(reservationId: number,updatedData: object) =>{ 
    try{
...
const deleteReservation  = async(reservationId: any) => {
    try{
        const reserve = await reservation.findByPk(reservationId);
        if(reserve){
